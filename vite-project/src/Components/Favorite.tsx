...
function Favorite(){
    ...
    const queryParams:string = favItemsId.map(id => `id=${id}`).join("&");
     useEffect(()=>{
        async function getProductsData(){
          const rec=await fetch(`${API_URL}/stationery?ids=${queryParams}`)
    ...
    <>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0d152d] bg-opacity-50 dark:bg-opacity-80 dark:bg-darkshadow">
            <div className="overflow-y-scroll scrollbar-thin scrollbar-thumb-light3 hover:scrollbar-thumb-btnDark scrollbar-track-light1 relative w-11/12  max-w-[90vw] h-[90vh]  rounded-lg shadow-lg dark:shadow-darkshadow  p-8  bg-light1 dark:bg-dark2 flex items-center justify-center"> 
                <button className="xBtn"  onClick={()=>setIsOpen(false)}><FaTimes className="icon"/></button>
                <div className="favList flex flex-wrap gap-2 justify-around ">
